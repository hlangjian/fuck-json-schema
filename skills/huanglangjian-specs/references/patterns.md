...
5. **Path parameters** use `{name}` syntax and are declared in `variables`. Path param types are restricted to `SimpleType` (primitives). If omitted, defaults to `string()`.
...
9. **Tags**: `RouterModel.tag ?? RouterModel.id` is the OpenAPI tag. Only set `tags` on `route()` for additional tags.
10. **Use `router()` factory** instead of plain object literals for `RouterModel`.
