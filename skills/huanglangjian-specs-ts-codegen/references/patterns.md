...
4. **Configuration generation** supports nested `record`, `taggedUnion`, `union`, `array`, `set`, and `enums`. Uses pure Zod with env parameterization (`getConfig(env = process.env)`). Config model types are also generated in `models.ts`.
...
6. **Tags**: `RouterModel.name` is automatically used as the OpenAPI tag. Do not set `tags` on individual routes unless additional tags are needed.
7. **Use `routerModel()`** instead of plain object literals for RouterModel.
