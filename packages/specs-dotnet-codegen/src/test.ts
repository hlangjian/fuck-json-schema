...
const Warehouse = record({
  ...
  title: "仓库",
  description: "仓库信息",
  examples: [{ id: 1, name: "默认仓库", location: "北京", capacity: 1000, created_at: "2024-01-01T00:00:00Z" }],
  properties: {
    ...
    created_at: datetime({ description: "创建时间" }),
  },
})
...
const OldWarehouse = record({
  ...
  properties: {
    name: string({ description: "仓库名称" }),
    legacy_code: string({ description: "旧编码", deprecated: true }),
  },
})
  ...
  properties: {
    ...
    auth: union({
      ...
      variants: {
        ...
        cert: record({
          id: "AuthCert",
          properties: { method: literal("cert"), cert_file: string(), key_file: string() },
        }),
      },
    }),
  },
...
const ServerConfig = record({
  ...
  optional: ["log_level", "tags", "allowed_ports", "cache"],
})
...
const serverFiles = generateDotnetServer({
  ...
  projectName: "WarehouseApi.Server",
  project: { version: "1.0.0", description: "Warehouse management API" },
...
const clientFiles = generateDotnetClient({
  routers: [warehousesRouter],
  namespace: "WarehouseApi.Client",
  projectName: "WarehouseApi.Client",
  project: { version: "1.0.0", description: "Warehouse management API client" },
})
