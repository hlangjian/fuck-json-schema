...
export const SimpleTypes = ["array", "boolean", "integer", "null", "number", "object", "string"] as const

export type SimpleType = (typeof SimpleTypes)[number]

export const MetaKeys: string[] = [
  "title",
  "description",
  "default",
  "deprecated",
...
export function parseNodeType(schema: JsonSchema): Map<JsonSchema, SimpleType[]> {
  ...
  const parse = (node: JsonSchema = {}) => {
    ...
    for (const key of ["prefixItems", "allOf", "anyOf", "oneOf"] as const)
      if (node[key]) for (const sub of node[key]) parse(sub)

    for (const key of ["$defs", "properties", "patternProperties", "dependentSchemas"] as const)
      if (node[key]) for (const subkey in node[key]) parse(node[key][subkey])
  }
